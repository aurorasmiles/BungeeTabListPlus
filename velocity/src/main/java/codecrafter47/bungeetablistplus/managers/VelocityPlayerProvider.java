...
public class VelocityPlayerProvider implements PlayerProvider {
    ...
    public VelocityPlayer onPlayerConnected(Player player) {
        ...
        mainThread.execute(() -> {
            velocityPlayer.getLocalDataCache().updateValue(BTLPBungeeDataKeys.DATA_KEY_CLIENT_VERSION, version);
            velocityPlayer.getLocalDataCache().updateValue(BTLPBungeeDataKeys.DATA_KEY_CLIENT_VERSION_BELOW_1_8, version_below_1_8);
